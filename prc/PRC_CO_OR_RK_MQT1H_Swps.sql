...
IF  EXISTS (SELECT 1
FROM sysobjects
where id = object_id(N'dbo.PRC_CO_OR_RK_MQT1H_Swps') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	DROP PROCEDURE dbo.PRC_CO_OR_RK_MQT1H_Swps
PRINT '<<<<< DROP Stored Procedure - "Kustom.dbo.PRC_CO_OR_RK_MQT1H_Swps" >>>>>'
GO
CREATE PROCEDURE [dbo].[PRC_CO_OR_RK_MQT1H_Swps]
	(
...
*/
	...
	DECLARE @COP INT
	SELECT @COP = Currencies_Id
	FROM KplusLocal..Currencies WITH(NOLOCK)
	WHERE Currencies_ShortName IN ('COP')
